...
const PAS_Base      = $F00;
  ...
  procedure PAS_SetProgressionRust(Eventer:TPurifyEvent);
   function PAS_NoCommentsRust(RE:TStrings):real;
...
label Mark;
begin
  PAS_NoCommentsRust := -1;
  s := RE.Text;
  LenStart:=Length(s);
  if LenStart = 0 then
    begin
      PAS_NoCommentsRust := 100;
      Exit;
    end;
  r:='';
  ...
  PAS_NoCommentsRust := Length(RE.Text)/LenStart*100;
end;
