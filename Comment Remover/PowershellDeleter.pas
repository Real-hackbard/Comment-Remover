...
const PAS_Base      = $F00;
  ...
  procedure PAS_SetProgressionPowershell(Eventer:TPurifyEvent);
   function PAS_NoCommentsPowershell(RE:TStrings):real;
...
label Mark;
begin
  PAS_NoCommentsPowershell := -1;
  s := RE.Text;
  LenStart:=Length(s);
  if LenStart = 0 then
    begin
      PAS_NoCommentsPowershell := 100;
      Exit;
    end;
  r:='';
    ...
    begin
      ...
      if (not Strng) and (c<>'''') and (not CmtLine) then
        begin
          ...
          // Here everything in the area of "<#..#>" is deleted
          if L2='<#' then
            begin
              r:=L1;
              Comment:=true;
  ...
  PAS_NoCommentsPowershell := Length(RE.Text)/LenStart*100;
end;
