...
const PAS_Base      = $F00;
  ...
  procedure PAS_SetProgressionhtml(Eventer:TPurifyEvent);
   function PAS_NoCommentshtml(RE:TStrings):real;
...
label Mark;
begin
  PAS_NoCommentshtml := -1;
  s := RE.Text;
  LenStart:=Length(s);
  if LenStart = 0 then
    begin
      PAS_NoCommentshtml := 100;
      Exit;
    end;
  r:='';
  ...
  PAS_NoCommentshtml := Length(RE.Text)/LenStart*100;
end;
