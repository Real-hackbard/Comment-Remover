...
const PAS_Base      = $F00;
  ...
  procedure PAS_SetProgressionRuby(Eventer:TPurifyEvent);
   function PAS_NoCommentsRuby(RE:TStrings):real;
...
label Mark;
begin
  PAS_NoCommentsRuby := -1;
  s := RE.Text;
  LenStart:=Length(s);
  if LenStart = 0 then
    begin
      PAS_NoCommentsRuby := 100;
      Exit;
    end;
  r:='';
  ...
  PAS_NoCommentsRuby := Length(RE.Text)/LenStart*100;
end;
