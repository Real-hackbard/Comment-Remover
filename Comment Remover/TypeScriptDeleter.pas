...
const PAS_Base      = $F00;
  ...
  procedure PAS_SetProgressionTypeScript(Eventer:TPurifyEvent);
   function PAS_NoCommentsTypeScript(RE:TStrings):real;
...
label Mark;
begin
  PAS_NoCommentsTypeScript := -1;
  s := RE.Text;
  LenStart:=Length(s);
  if LenStart = 0 then
    begin
      PAS_NoCommentsTypeScript := 100;
      Exit;
    end;
  r:='';
  ...
  PAS_NoCommentsTypeScript := Length(RE.Text)/LenStart*100;
end;
