...
const PAS_Base      = $F00;
  ...
  procedure PAS_SetProgressionMatlab(Eventer:TPurifyEvent);
   function PAS_NoCommentsMatlab(RE:TStrings):real;
...
label Mark;
begin
  PAS_NoCommentsMatlab := -1;
  s := RE.Text;
  LenStart:=Length(s);
  if LenStart = 0 then
    begin
      PAS_NoCommentsMatlab := 100;
      Exit;
    end;
  r:='';
  ...
  PAS_NoCommentsMatlab := Length(RE.Text)/LenStart*100;
end;
