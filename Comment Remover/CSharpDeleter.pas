...
const PAS_Base      = $F00;
  ...
  procedure PAS_SetProgressionCSharp(Eventer:TPurifyEvent);
   function PAS_NoCommentsCSharp(RE:TStrings):real;
...
label Mark;
begin
  PAS_NoCommentsCSharp := -1;
  s := RE.Text;
  LenStart:=Length(s);
  if LenStart = 0 then
    begin
      PAS_NoCommentsCSharp := 100;
      Exit;
    end;
  r:='';
  ...
  for x:=1 to LenStart do
    begin
      ...
      // Delete the contents of the double slash
      if (not Strng) and (c<>'''') and (not CmtLine) then
        begin
          L1:=Copy(r,1,Length(r)-2);
          L2:=Copy(r,Length(r)-1,2);
          // Here everything after the specified string is deleted, in this case slash
          if L2='//' then
            begin
              CmtLine:=true;
              r:=L1;
  ...
  PAS_NoCommentsCSharp := Length(RE.Text)/LenStart*100;
end;
