...
const PAS_Base      = $F00;
  ...
  procedure PAS_SetProgressionSwift(Eventer:TPurifyEvent);
   function PAS_NoCommentsSwift(RE:TStrings):real;
...
label Mark;
begin
  PAS_NoCommentsSwift := -1;
  s := RE.Text;
  LenStart:=Length(s);
  if LenStart = 0 then
    begin
      PAS_NoCommentsSwift := 100;
      Exit;
    end;
  r:='';
  ...
  PAS_NoCommentsSwift := Length(RE.Text)/LenStart*100;
end;
